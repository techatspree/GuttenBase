...
/**
 * Export schema information and data into executable JAR file. Since it is in ZIP file format the resulting file may as well be inspected
 * with a ZIP tool. The structure of the ZIP is based on the structure of a data base.
 * 
 * <p>
 * &copy; 2012 akquinet tech@spree
 * </p>
 * 
 * @Uses-Hint {@link ZipExporterClassResourcesHint} to add custom classes to the generated JAR and configure the META-INF/MANIFEST.MF
 *            Main-Class entry
...
public class ZipExporter implements Exporter {
	...
	private void writeIndexEntries(final TableMetaData tableMetaData) throws IOException {
		final String indexPath = ZipConstants.PREFIX + tableMetaData.getTableName() + ZipConstants.PATH_SEPARATOR + ZipConstants.INDEX_NAME
				+ ZipConstants.PATH_SEPARATOR;
	...
	private void writeColumnEntries(final TableMetaData tableMetaData) throws IOException {
		final String columnPath = ZipConstants.PREFIX + tableMetaData.getTableName() + ZipConstants.PATH_SEPARATOR + ZipConstants.COLUMN_NAME
				+ ZipConstants.PATH_SEPARATOR;
	...
	private void addClassesToJar(final ConnectorRepository connectorRepository, final String connectorId,
		...
		final List<Class<?>> classResources = zipExporterClassResources.getClassResources();
		for (final Class<?> clazz : classResources) {
