...
public class DatabaseSchemaScriptCreatorTest {
	...
	private DatabaseMetaData createDatabaseMetaData() {
		...
		final ForeignKeyMetaDataBuilder foreignKeyMetaDataBuilder1 = new ForeignKeyMetaDataBuilder(tableBuilder1).setForeignKeyName("FK_NAME")
				.setReferencingColumn(tableBuilder1.getColumn("NAME")).setReferencedColumn(tableBuilder2.getColumn("NAME"));
		final ForeignKeyMetaDataBuilder foreignKeyMetaDataBuilder2 = new ForeignKeyMetaDataBuilder(tableBuilder2).setForeignKeyName("FK_NAME")
				.setReferencingColumn(tableBuilder1.getColumn("NAME")).setReferencedColumn(tableBuilder2.getColumn("NAME"));

		tableBuilder1.addForeignKey(foreignKeyMetaDataBuilder1);
		tableBuilder2.addForeignKey(foreignKeyMetaDataBuilder2);
	...
	private TableMetaDataBuilder createTable(final int index, final DatabaseMetaDataBuilder databaseMetaDataBuilder) {
		final TableMetaDataBuilder tableMetaDataBuilder = new TableMetaDataBuilder(databaseMetaDataBuilder).setTableName("MY_TABLE" + index);
